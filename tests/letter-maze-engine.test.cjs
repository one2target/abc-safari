...
assert.equal(engine.move(scene,state,'p02'),'correct-empty');
assert.equal(engine.move(scene,state,'p03'),'correct-empty');
assert.equal(state.currentCell,'p03');

while(!engine.roundComplete(scene,state))assert.ok(['correct-letter','correct-empty','round-complete'].includes(engine.move(scene,state,engine.nextPathCell(scene,state))));
...
for(let round=1;round<scene.rounds.length;round++){
  let result;
  while(!engine.roundComplete(scene,state))result=engine.move(scene,state,engine.nextPathCell(scene,state));
...
audible.run("selectLetterMazeCell('p02',DEF_LETTER_MAZE.id);selectLetterMazeCell('p03',DEF_LETTER_MAZE.id);selectLetterMazeCell('p04',DEF_LETTER_MAZE.id);selectLetterMazeCell('p05',DEF_LETTER_MAZE.id);selectLetterMazeCell('p06',DEF_LETTER_MAZE.id)");
