...
function createContext({saved=null,blocked=false,audioResult='end',extraLetter=false}={}){
 ...
 const document={querySelector:sel=>sel==='#find-object-image'&&!node('#main').innerHTML.includes('id="find-object-image"')?null:node(sel),querySelectorAll:()=>[],addEventListener(){},activeElement:node('active'),hidden:false,createTextNode:s=>s,createElement:tag=>node('new'+tag),head:node('head'),body:node('body')};
